...
Lo malo: no está listo para "millones de documentos / miles de usuarios" tal como pide el enunciado. Los problemas no
están en el estilo, sino en decisiones de datos y de escalabilidad que se rompen exactamente cuando crece el
volumen:

1. No existe índice ANN (HNSW/IVFFlat) sobre fragment.vector. Toda búsqueda vectorial es un KNN exacto = full scan de la
   tabla fragment. Con millones de fragmentos, cada query semántica recorre toda la tabla. Es el cuello de  
   botella #1.
2. No hay migraciones (Alembic). El esquema vive parcialmente en el ORM. Sin control de versiones de esquema, sin forma
   reproducible de crear índices, constraints o columnas. Schema drift garantizado.
3. La eliminación es incompleta y deja datos huérfanos en 3 sistemas: borra (soft) filas en Postgres, pero no toca
   Neo4j, no borra el objeto en MinIO, y no purga vectores. Fuga de almacenamiento permanente + riesgo de
...
Veredicto: sólida prueba de concepto / MVP avanzado. No apto para producción a la escala declarada ni para entorno
militar/institucional sin abordar los hallazgos críticos.
...
delete_document_service.py solo llama soft_delete_fragments_by_document_id + soft_delete_document_by_id. No hay ninguna
operación DETACH DELETE/delete_by_document en los repos de grafo (grep confirmado), ni delete_document de
MinIO, ni borrado físico de vectores.

Consecuencias:

- Neo4j: entidades/relaciones conservan el document_id borrado en source_document_ids para siempre. Datos huérfanos + el
  grafo sigue sirviendo hechos derivados de documentos eliminados (riesgo de confidencialidad).
- MinIO: el binario original nunca se borra → fuga de almacenamiento ilimitada + el "derecho al olvido"/desclasificación
  es imposible.
- pgvector: el soft-delete deja el vector en la tabla; el índice (cuando exista) y el full-scan actual siguen pagando su
  coste. Bloat permanente.
...
No existe columna tenant_id en ninguna tabla ni etiqueta de tenant en Neo4j (grep). Todos los tenants comparten una sola
tabla fragment y un solo grafo. El aislamiento depende 100% de un servicio externo
(document_collection_catalog). Un bug en ese filtro = fuga cross-tenant total. Para un entorno militar/institucional
esto es inaceptable sin RLS o particionado por tenant.
...
- FKs cross-service. fragment.document_id con ondelete=CASCADE apunta a document, y existe FK a chat en otra base de
  servicio (documentado en orm/document.py:14). Acoplamiento de datos entre servicios.
...
- Híbrido secuencial. En fragment_query_service.py:147-177 las queries semánticas se embeben con asyncio.gather (bien)
  pero las búsquedas vectoriales y BM25 se ejecutan en bucles await secuenciales, no en paralelo. Llamadas a
...
- Se traen vectores completos por la red sin usarlos. Tanto get_most_similar_fragments como BM25 hacen SELECT ...
  vector ... y construyen Fragment(vector=...), pero la respuesta nunca usa el vector. Son ~1024 floats × k       
...
- Solo fixed-size recursive (recursive_text_splitter.py) en producción. No hay splitting estructural (por
  encabezado/sección/tabla) pese a usar Docling como reader. Se pierde la estructura del documento antes de
  chunkear →    
  fragmentación que parte tablas, listas y cláusulas legales por la mitad.
- Metadata de chunk casi inexistente. orm/fragment.py solo tiene fragment_index. Falta: página, sección, jerarquía,
  offset char/inicio-fin, parent/heading, bounding box. Imposible citar "página 14, art. 3" en un RAG
  legal/militar.
- split_size/split_overlap nunca se persisten. La ingesta (document_ingestion_service.py:300-302) escribe
  text_cleaner_type/splitter_type/embedder_type pero no split_size/split_overlap (existen como columnas nulables
  en       
  orm/document.py:35-36 y quedan NULL). Se pierde la trazabilidad necesaria para reindexar de forma determinista.
- Overlap por defecto 50 tokens sobre chunks de 512 (text_splitter_settings.py:42-43) — ~10%, razonable, pero al ser
  ciego a estructura el overlap no garantiza no cortar entidades clave.
