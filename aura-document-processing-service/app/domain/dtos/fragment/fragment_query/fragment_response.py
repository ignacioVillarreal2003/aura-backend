...
class _Document(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=MAX_CHARS)
    description: Optional[str] = Field(default=None, min_length=1, max_length=MAX_CHARS)
    type: Optional[DocumentType] = Field(default=None)
    category: Optional[str] = Field(default=None, min_length=1, max_length=MAX_CHARS)
...
class FragmentResponse(BaseModel):
    ...
    content: str = Field(..., min_length=1, max_length=MAX_LARGE_CHARS)
    fragment_index: int = Field(..., ge=0, le=MAX_FRAGMENT_INDEX)
    summary: Optional[str] = Field(default=None, min_length=1, max_length=MAX_LARGE_CHARS)
    ...
    def sanitize_content(cls, v: object) -> object:
        ...
        return content[:MAX_LARGE_CHARS]
    ...
    def sanitize_summary(cls, v: object) -> Optional[str]:
        if not isinstance(v, str):
            return v
        summary = v.strip()
        return summary[:MAX_LARGE_CHARS] if summary else None
    ...
    def sanitize_topics(cls, v: object) -> Optional[list[str]]:
        if not isinstance(v, list):
            return v
        cleaned = [t.strip()[:MAX_CHARS] for t in v if isinstance(t, str) and t.strip()]
    ...
    def validate_entities(self) -> "FragmentResponse":
        ...
        for key, value in self.entities.items():
            ...
            if len(key) > MAX_CHARS:
                raise ValueError(
                    f"Entity key '{key[:40]}...' exceeds {MAX_CHARS} characters."
                )
