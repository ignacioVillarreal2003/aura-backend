...
from app.infrastructure.http.authentication_provider.service_authentication_provider import (
    ServiceAuthError,
    ServiceAuthSuccess,
    evaluate_service_authentication,
)
...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        if self._is_excluded_path(request.url.path):
            logger.debug("Path excluded from authentication", extra={"path": request.url.path})
            return await call_next(request)

        service_outcome = evaluate_service_authentication(request)
        if isinstance(service_outcome, ServiceAuthSuccess):
            request.state.authenticated_user = service_outcome.user
            logger.debug(
                "Service-to-service authentication accepted",
                extra={"user_id": service_outcome.user.id, "path": request.url.path},
            )
            return await call_next(request)
        if isinstance(service_outcome, ServiceAuthError):
            return JSONResponse(
                status_code=service_outcome.status_code,
                content={"detail": service_outcome.detail, "error": service_outcome.error},
            )
    ...
    async def _authenticate(
        ...
        try:
            authentication_provider = request.app.state.authentication_provider
