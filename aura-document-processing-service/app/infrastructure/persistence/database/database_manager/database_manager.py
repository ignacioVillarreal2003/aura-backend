...
from app.infrastructure.persistence.database.database_manager.exceptions.database_manager_exception import (
    ...
    DatabaseNotInitializedException, DatabaseSessionException
)
...
class DatabaseManager(DatabaseManagerInterface):
    def __init__(
        ...
        self._database_manager_settings = database_manager_settings or DatabaseManagerSettings()
    ...
    async def initialize(
        ...
        logger.info(
            ...
            extra={
                "url": self._database_manager_settings.url_safe,
                "pool_persistent_connections": self._database_manager_settings.pool_persistent_connections,
                "pool_overflow_connections": self._database_manager_settings.pool_overflow_connections,
            }
        )

        try:
            self._engine = create_async_engine(
                self._database_manager_settings.url,
                echo=self._database_manager_settings.echo_sql,
                pool_size=self._database_manager_settings.pool_persistent_connections,
                max_overflow=self._database_manager_settings.pool_overflow_connections,
                pool_recycle=self._database_manager_settings.pool_recycle_seconds,
                pool_pre_ping=self._database_manager_settings.pool_liveness_probe,
                pool_timeout=self._database_manager_settings.pool_checkout_timeout_seconds,
                pool_reset_on_return="rollback",
                isolation_level="READ COMMITTED",
                connect_args=self._database_manager_settings.get_connect_args(),
                echo_pool=self._database_manager_settings.echo_sql
            )
            ...
            logger.info(
                ...
                extra={
                    "pool_persistent_connections": self._database_manager_settings.pool_persistent_connections,
                    "pool_overflow_connections": self._database_manager_settings.pool_overflow_connections,
                    "ssl_enabled": self._database_manager_settings.ssl_enabled
                }
            )
    ...
    async def session(
        ...
        if (not self._is_initialized
            ...
            raise DatabaseNotInitializedException("DatabaseManager is not initialized. Call initialize() first.")
        ...
        try:
            ...
            return {
                ...
                "settings": {
                    "pool_persistent_connections": self._database_manager_settings.pool_persistent_connections,
                    "pool_overflow_connections": self._database_manager_settings.pool_overflow_connections,
                    "pool_checkout_timeout_seconds": self._database_manager_settings.pool_checkout_timeout_seconds,
                    "ssl_enabled": self._database_manager_settings.ssl_enabled
                }
            }
    ...
    def _setup_event_listeners(
        ...
        def on_checkout(
            ...
            if self._database_manager_settings.query_logging_enabled:
