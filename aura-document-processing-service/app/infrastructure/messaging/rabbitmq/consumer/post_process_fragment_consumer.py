...
from app.application.services.fragment.post_process_fragment_processor.post_process_fragment_processor_interface import (
    ...
    async def _handle_message(
        ...
        try:
            envelope = MessageEnvelope.from_bytes(
                data=body,
                command_type=PostProcessFragmentJobCommand,
                retry_count=retry_count,
            )
        ...
        try:
            ...
            await message.ack()
            logger.info(
                "The fragment post-process job message was acknowledged.",
                extra={"message_id": envelope.message_id, "job_id": job_id},
            )
        except Exception:
            logger.exception(
                "The fragment post-process job handler failed; negative-acknowledging for dead-letter retry.",
