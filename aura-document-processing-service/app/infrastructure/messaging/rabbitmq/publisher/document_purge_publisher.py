...
from app.infrastructure.messaging.rabbitmq.dtos.commands.document_purge_command import (
...
class DocumentPurgePublisher(DocumentPurgePublisherInterface):
    def __init__(
            self,
    ...
    async def publish(
        ...
        if self._outbox_lite is not None:
            await self._outbox_lite.publish_or_enqueue(
                ...
                event_type="document_purge",
                aggregate_id=str(document_id),
                routing_key=self._settings.document_purge_queue,
        ...
        logger.info(
            "A document-purge command was published.",
            extra={
                "document_id": document_id,
                "message_id": envelope.message_id,
            },
