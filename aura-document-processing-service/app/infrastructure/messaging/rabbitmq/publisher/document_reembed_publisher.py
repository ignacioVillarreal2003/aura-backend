...
from app.infrastructure.messaging.rabbitmq.dtos.commands.document_reembed_command import (
...
class DocumentReembedPublisher(DocumentReembedPublisherInterface):
    def __init__(
            self,
    ...
    async def publish(
        ...
        if self._outbox_lite is not None:
            await self._outbox_lite.publish_or_enqueue(
                ...
                event_type="document_reembed",
                aggregate_id=str(document_id),
                routing_key=self._settings.document_reembed_queue,
...
async def get_document_reembed_publisher(
        request: Request,
) -> DocumentReembedPublisherInterface:
    publisher = getattr(request.app.state, "document_reembed_publisher", None)
    if publisher is None:
        logger.error("DocumentReembedPublisher is not registered on the application state.")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Document re-embed publisher is not available.",
        )
    return publisher
