...
class DocumentQueryServiceRequestValidator:
    ...
    def _validate_question(
        ...
        if length < self._document_query_service_settings.min_question_length:
            raise DocumentQueryInvalidRequestException(
                f"Question length ({length}) is below the minimum "
                f"({self._document_query_service_settings.min_question_length})"
            )

        if length > self._document_query_service_settings.max_question_length:
            raise DocumentQueryInvalidRequestException(
                f"Question length ({length}) exceeds the maximum "
                f"({self._document_query_service_settings.max_question_length})"
            )

    def _validate_max_fragments(
            self,
            max_fragments: int
    ) -> None:
        if max_fragments < 1:
            raise DocumentQueryInvalidRequestException("max_context_fragments must be at least 1")

        if max_fragments > self._document_query_service_settings.max_fragments:
            raise DocumentQueryInvalidRequestException(
                f"max_context_fragments ({max_fragments}) exceeds the configured limit "
                f"({self._document_query_service_settings.max_fragments})"
            )
