...
from app.application.services.document_retrieve_service.exceptions.document_retrieve_service_exceptions import (
    DocumentContextServiceException,
    EmbeddingException,
...
class DocumentRetrieveService(DocumentRetrieveServiceInterface):
    def __init__(
            self,
        ...
        self._document_retrieve_service_settings = document_retrieve_service_settings
        ...
        try:
            try:
                embedder = self._embedder_factory.get_embedder(
                    method=self._document_retrieve_service_settings.embedder_type
                )
            ...
            context_fragments = [fragment.content for fragment in fragments]

            self._question_queries += 1
            self._total_fragments_returned += len(context_fragments)
            ...
            logger.info(
                ...
                extra={
                    "num_fragments": len(context_fragments),
            ...
            raise DocumentContextServiceException(f"Failed to retrieve context fragments: {str(e)}") from e
    ...
    async def retrieve_context_fragments_by_document(
        ...
        try:
            ...
            raise DocumentContextServiceException(f"Failed to retrieve document fragments: {str(e)}") from e
