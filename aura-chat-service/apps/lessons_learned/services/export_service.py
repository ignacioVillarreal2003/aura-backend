...
from apps.lessons_learned.exceptions import LessonsLearnedExportException
...
_CATEGORY_LABELS = {
    LessonsLearnedItem.Category.SUSTAIN: "Sostener",
    LessonsLearnedItem.Category.IMPROVE: "Mejorar",
    LessonsLearnedItem.Category.RECOMMENDATION: "Recomendación",
}
_CATEGORY_ORDER = [
    LessonsLearnedItem.Category.SUSTAIN,
    LessonsLearnedItem.Category.IMPROVE,
    LessonsLearnedItem.Category.RECOMMENDATION,
]
...
def _grouped_items(ll: LessonsLearned) -> dict:
...
def generate_lessons_learned_pdf(ll: LessonsLearned) -> bytes:
    mode_label = "Con documentos de contexto" if ll.mode == LessonsLearned.Mode.RAG else "Directo"
...
def generate_lessons_learned_markdown(ll: LessonsLearned) -> str:
    grouped = _grouped_items(ll)
