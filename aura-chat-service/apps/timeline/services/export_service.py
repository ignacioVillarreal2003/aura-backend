...
from apps.timeline.exceptions import TimelineExportException
...
def _build_pdf(html_content: str) -> bytes:
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            logger.error("Timeline PDF generation timed out after %ds", _PDF_TIMEOUT_SECONDS)
            raise TimelineExportException()


def generate_timeline_pdf(timeline: Timeline) -> bytes:
    events = list(timeline.events.all())
    mode_label = "Con documentos de contexto" if timeline.mode == Timeline.Mode.RAG else "Directo"
    created = html.escape(_fmt_dt(timeline.created_at))

    events_html = ""
    for event in events:
        when = html.escape(_event_when(event))
        title = html.escape(event.event)
...
def generate_timeline_markdown(timeline: Timeline) -> str:
    events = list(timeline.events.all())
    ...
    for event in events:
        lines.append(f"### {_event_when(event)} — {event.event}")
