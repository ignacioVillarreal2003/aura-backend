...
from apps.decision_brief.exceptions import DecisionBriefExportException
...
def generate_decision_brief_pdf(brief: DecisionBrief) -> bytes:
    options = list(brief.options.all())
    mode_label = "Con documentos de contexto" if brief.mode == DecisionBrief.Mode.RAG else "Directo"
...
def generate_decision_brief_markdown(brief: DecisionBrief) -> str:
    options = list(brief.options.all())
