...
_TYPE_LABELS = {
    QuizQuestion.Type.SINGLE: "Opción única",
    QuizQuestion.Type.MULTIPLE: "Opción múltiple",
    QuizQuestion.Type.BOOLEAN: "Verdadero/Falso",
    QuizQuestion.Type.OPEN: "Respuesta abierta",
}
...
def _build_pdf(html_content: str) -> bytes:
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            logger.error("Quiz PDF generation timed out after %ds", _PDF_TIMEOUT_SECONDS)
            raise QuizExportException()


def generate_quiz_pdf(quiz: Quiz, *, with_answers: bool = True) -> bytes:
    questions = list(quiz.questions.all())
    mode_label = "Con documentos de contexto" if quiz.mode == Quiz.Mode.RAG else "Directo"
    created = html.escape(_fmt_dt(quiz.created_at))
    pass_label = f"{quiz.passing_score}%" if quiz.passing_score is not None else "—"

    questions_html = ""
    for idx, question in enumerate(questions, start=1):
        type_label = html.escape(_TYPE_LABELS.get(question.type, question.type))
...
def generate_quiz_markdown(quiz: Quiz, *, with_answers: bool = True) -> str:
    questions = list(quiz.questions.all())
    pass_label = f"{quiz.passing_score}%" if quiz.passing_score is not None else "—"
    ...
    for idx, question in enumerate(questions, start=1):
        ...
        lines.append(f"### {idx}. {question.question} _({type_label})_")
        for opt in question.options.all():
