...
from apps.document_collection.exceptions import CollectionNotFoundException
...
from apps.document_collection.repositories import document_collection_repository
...
class DocumentCollectionService:
    @staticmethod
    def list_document_collections(user: AuthenticatedUser) -> QuerySet[DocumentCollection]:
        _permissions_gate(user, LIST_DOCUMENT_COLLECTIONS)
        return document_collection_repository.list_active()

    @staticmethod
    @transaction.atomic
    def create_document_collection(user: AuthenticatedUser, name: str) -> DocumentCollection:
    ...
    @staticmethod
    def get_document_collection(user: AuthenticatedUser, document_collection_id: int) -> DocumentCollection:
    ...
    @staticmethod
    @transaction.atomic
    def delete_document_collection(user: AuthenticatedUser, document_collection_id: int) -> None:
