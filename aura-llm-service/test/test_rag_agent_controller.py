...
_RESPONSE = AgentResponse(
    messages=[
        ...
        {"role": "ai", "content": "Según el contrato, las penalidades son las siguientes."},
    ]
)
...
class TestRagAgentAuth:
    ...
    def test_missing_permission_returns_403(self, client, service_headers, mock_rag_agent_service):
        response = client.post(URL, json=VALID_BODY, headers=service_headers(permissions=[]))
        assert response.status_code == 403

    def test_wrong_permission_returns_403(self, client, service_headers, mock_rag_agent_service):
    ...
    def test_last_message_not_human_returns_422(self, client, auth_headers, mock_rag_agent_service):
        body = {
            "messages": [
                ...
                {"role": "ai", "content": "¿En qué te ayudo?"},
            ]
        }
        response = client.post(URL, json=body, headers=auth_headers)
        assert response.status_code == 422
