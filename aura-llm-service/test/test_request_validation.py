...
class TestDocumentQuestionRequest:
    def test_valid_single_human_message(self):
        req = DocumentQuestionRequest(messages=[_HUMAN_MSG])
        assert req.messages[0].role.value == "human"

    def test_valid_with_history(self):
        req = DocumentQuestionRequest(messages=[_HUMAN_MSG, _AI_MSG, _HUMAN_MSG])
        assert len(req.messages) == 3

    def test_last_message_must_be_human(self):
        with pytest.raises(ValidationError):
            DocumentQuestionRequest(messages=[_HUMAN_MSG, _AI_MSG])

    def test_empty_messages_raises(self):
        with pytest.raises(ValidationError):
            DocumentQuestionRequest(messages=[])

    def test_blank_message_content_raises(self):
        with pytest.raises(ValidationError):
            DocumentQuestionRequest(messages=[{"role": "human", "content": "   "}])

    def test_optional_chat_id_defaults_to_none(self):
        req = DocumentQuestionRequest(messages=[_HUMAN_MSG])
...
class TestDocumentSummaryRequest:
    def test_valid_single_document_id(self):
        req = DocumentSummaryRequest(document_ids=[1])
        assert req.document_ids == [1]

    def test_valid_multiple_ids(self):
        req = DocumentSummaryRequest(document_ids=[1, 2, 3])
        assert len(req.document_ids) == 3

    def test_empty_list_raises(self):
        with pytest.raises(ValidationError):
            DocumentSummaryRequest(document_ids=[])

    def test_duplicate_ids_raise(self):
        with pytest.raises(ValidationError):
            DocumentSummaryRequest(document_ids=[1, 1, 2])

    def test_zero_id_raises(self):
        with pytest.raises(ValidationError):
            DocumentSummaryRequest(document_ids=[0])
    ...
    def test_too_many_ids_raises(self):
        with pytest.raises(ValidationError):
            DocumentSummaryRequest(document_ids=list(range(1, 52)))
...
class TestAgentRequest:
    def test_valid_single_human_message(self):
        req = AgentRequest(messages=[_HUMAN_MSG])
        assert len(req.messages) == 1

    def test_last_message_must_be_human(self):
        with pytest.raises(ValidationError):
            AgentRequest(messages=[_HUMAN_MSG, _AI_MSG])
    ...
    def test_multi_turn_ends_with_human(self):
        req = AgentRequest(messages=[_HUMAN_MSG, _AI_MSG, _HUMAN_MSG])
...
class TestDocumentActionRequest:
    def test_valid_request(self):
        req = DocumentActionRequest(document_ids=[1, 2], instruction="Resumir el contenido.")
        assert req.instruction == "Resumir el contenido."

    def test_blank_instruction_raises(self):
        with pytest.raises(ValidationError):
            DocumentActionRequest(document_ids=[1], instruction="   ")

    def test_duplicate_document_ids_raise(self):
        with pytest.raises(ValidationError):
            DocumentActionRequest(document_ids=[1, 1], instruction="Resumir.")

    def test_zero_document_id_raises(self):
        with pytest.raises(ValidationError):
            DocumentActionRequest(document_ids=[0], instruction="Resumir.")

    def test_empty_document_ids_raises(self):
        with pytest.raises(ValidationError):
            DocumentActionRequest(document_ids=[], instruction="Resumir.")

    def test_strips_whitespace_from_instruction(self):
        req = DocumentActionRequest(document_ids=[1], instruction="  Resumir.  ")
