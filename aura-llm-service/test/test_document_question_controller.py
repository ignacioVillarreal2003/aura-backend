...
class TestDocumentQuestionAuth:
    ...
    def test_missing_permission_returns_403(self, client, service_headers, mock_document_question_service):
        response = client.post(URL, json=VALID_BODY, headers=service_headers(permissions=[]))
        assert response.status_code == 403

    def test_wrong_permission_returns_403(self, client, service_headers, mock_document_question_service):
        response = client.post(URL, json=VALID_BODY, headers=service_headers(permissions=["LLM_AGENT"]))
        assert response.status_code == 403
...
class TestDocumentQuestionSuccess:
    ...
    def test_with_optional_chat_id(self, client, auth_headers, mock_document_question_service):
        mock_document_question_service.execute_document_question.return_value = _RESPONSE
        body = {**VALID_BODY, "chat_id": 10}
        response = client.post(URL, json=body, headers=auth_headers)
        assert response.status_code == 200
