...
class TestDocumentSummaryAuth:
    ...
    def test_missing_permission_returns_403(self, client, service_headers, mock_document_summary_service):
        response = client.post(URL, json=VALID_BODY, headers=service_headers(permissions=[]))
        assert response.status_code == 403

    def test_wrong_permission_returns_403(self, client, service_headers, mock_document_summary_service):
...
class TestDocumentSummarySuccess:
    def test_valid_request_returns_200(self, client, auth_headers, mock_document_summary_service):
        mock_document_summary_service.summarize_documents.return_value = _RESPONSE
