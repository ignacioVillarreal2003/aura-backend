...
class TestGraphExtractionAuth:
    ...
    def test_missing_permission_returns_403(self, client, service_headers, mock_graph_extraction_service):
        response = client.post(URL, json=VALID_BODY, headers=service_headers(permissions=[]))
        assert response.status_code == 403

    def test_wrong_permission_returns_403(self, client, service_headers, mock_graph_extraction_service):
