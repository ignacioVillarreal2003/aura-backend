...
class FragmentEnrichService(FragmentEnrichServiceInterface):
    def __init__(
        ...
        self._fragment_enrich_settings = fragment_enrich_service_settings or FragmentEnrichServiceSettings()
    ...
    def _truncate_content(self, content: str, user_id: int) -> str:
        if len(content) <= self._fragment_enrich_settings.max_content_chars:
            return content
        logger.info(
            "Truncating fragment content for enrichment",
            extra={
                "original_len": len(content),
                "max_chars": self._fragment_enrich_settings.max_content_chars,
                "user_id": user_id,
            },
        )
        return content[: self._fragment_enrich_settings.max_content_chars]
