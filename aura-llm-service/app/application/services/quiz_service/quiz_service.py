...
from app.domain.dtos.quiz.quiz_request import QuizGenerateRequest, QuizMode
from app.domain.dtos.quiz.quiz_response import (
...
def _parse_options(raw_options: list) -> list[QuizOption]:
    options: list[QuizOption] = []
    for entry in raw_options[:_MAX_OPTIONS]:
        if not isinstance(entry, dict):
            continue
        text = _clean(entry.get("text"), _MAX_OPTION_CHARS)
        if not text:
            continue
...
def _parse_questions(raw_questions: list) -> list[QuizQuestion]:
    questions: list[QuizQuestion] = []
    for entry in raw_questions[:_MAX_QUESTIONS]:
        if not isinstance(entry, dict):
            continue
        text = _clean(entry.get("question"), _MAX_QUESTION_CHARS)
        if not text:
            continue
...
def _fallback_questions(raw: str) -> tuple[str, str, int | None, list[QuizQuestion]]:
    lines = [ln.strip().lstrip("•-*0123456789.) ") for ln in raw.splitlines() if ln.strip()]
    questions = [
        QuizQuestion(question=ln[:_MAX_QUESTION_CHARS], type=QuizQuestionType.OPEN, options=[])
        for ln in lines[:_MAX_QUESTIONS]
        if ln
    ]
...
def _parse_llm_output(raw: str) -> tuple[str, str, int | None, list[QuizQuestion]]:
    try:
        ...
        return _fallback_questions(raw)
...
class QuizService(QuizServiceInterface):
    ...
    async def generate(
        ...
        try:
            ...
            if not questions:
                raise QuizServiceException(
                    "No se pudieron extraer preguntas de la respuesta del modelo.",
                    status_code=502,
                )
            ...
            raise QuizServiceException(
                "Error inesperado durante la generación del cuestionario.",
                status_code=500,
            ) from e
