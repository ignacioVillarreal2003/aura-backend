...
from app.application.services.document_summary_service.steps.map_chunks.map_chunks_prompt import (
    CHUNK_SUMMARY_HUMAN_PROMPT,
    SYSTEM_PROMPT,
)
from app.application.services.document_summary_service.steps.map_chunks.map_chunks_settings import MapChunksSettings
from app.infrastructure.document_context_provider.dtos.context_fragments_response import ContextFragmentResponse

logger = logging.getLogger(__name__)


class MapChunksPlugin(DocumentSummaryPlugin):
    def __init__(self, map_chunks_settings: Optional[MapChunksSettings] = None) -> None:
        self._settings = map_chunks_settings or MapChunksSettings()
    ...
    def should_run(
        ...
        return state.strategy == SummarizationStrategy.map_reduce and bool(state.fragments)
    ...
    async def run(
        ...
        chunks = self._create_chunks(state.fragments, self._settings.chunk_size)
        logger.debug(
            "Starting parallel chunk processing",
            extra={"chunk_count": len(chunks), "fragment_count": len(state.fragments)},
        )
        ...
        state.partial_summaries = successful
        logger.debug(
            "Map phase completed",
            extra={"partial_summary_count": len(successful)},
        )

    @staticmethod
    def _create_chunks(
            fragments: List[ContextFragmentResponse],
            chunk_size: int,
    ) -> List[List[ContextFragmentResponse]]:
        return [
    ...
    async def _process_chunk_with_retry(
            self,
            chunk_index: int,
            chunk: List[ContextFragmentResponse],
            resources: DocumentSummaryPipelineResources,
    ) -> str:
        ...
        async with self._semaphore:
            for attempt in range(max_attempts):
                try:
                    ...
                    summary = await self._summarize_chunk(chunk=chunk, resources=resources)
                    logger.debug(
    ...
    async def _summarize_chunk(
        ...
        llm_input = [
            ...
            HumanMessage(
                content=CHUNK_SUMMARY_HUMAN_PROMPT.format(fragments_joined=fragments_joined)
            ),
        ]

        llm = await resources.ollama_llm_facade.get_llm_base()
        summary = await resources.llm_invoker.call_llm_content(llm=llm, llm_input=llm_input)
