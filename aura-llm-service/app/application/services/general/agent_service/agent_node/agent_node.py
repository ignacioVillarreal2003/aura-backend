...
from app.application.services.general.agent_service.agent_node.agent_node_prompt_builder import AgentNodePromptBuilder
...
class AgentNode:
    def __init__(
            ...
            configuration: Optional[AgentNodeConfiguration] = None
    ) -> None:
    ...
    async def process(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(response, BaseMessage):
                raise TypeError(
                    f"LLM returned unexpected type: {type(response).__name__}"
                )
    ...
    async def _ensure_llm_initialized(self) -> None:
        ...
        try:
            ...
            logger.debug("LLM with tools initialized for agent node")
        except Exception as e:
            self._llm_initialization_failed = True
            logger.error("Failed to initialize LLM with tools for agent node", exc_info=True)
            raise RuntimeError("Failed to initialize LLM for agent node") from e
    ...
    def _extract_sentiment(agent_state: AgentState) -> Sentiment:
        ...
        try:
            ...
            logger.warning("Invalid sentiment value in state — defaulting to NEUTRAL")
