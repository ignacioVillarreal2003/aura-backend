...
from documents.services.document_service import upload_document_for_admin
...
class DocumentAdmin(AuditedAdminMixin, admin.ModelAdmin):
    ...
    def group_count(self, obj):
        return count_badge(obj.groups.count())
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            ...
            apply_audit_fields(obj, request.user.pk, is_create=False)
        ...
        response_payload = upload_document_for_admin(
        ...
        apply_audit_fields(obj, request.user.pk, is_create=True)
        super().save_model(request, obj, form, change)
        if obj.visible_to_all:
            obj.groups.clear()
        else:
